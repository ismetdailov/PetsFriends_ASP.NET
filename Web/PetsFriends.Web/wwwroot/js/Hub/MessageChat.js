...
var connection = new signalR.HubConnectionBuilder().withUrl("/MessageChat").build();
...
document.getElementById("sendButton").addEventListener("click", function (event) {
    let reciver = document.getElementById("reciver").textContent;
    ...
    if (message && images.length == 0 && files.length == 0) {
        connection.invoke("SendMessage", sender, reciver, message, group).catch(function (err) {
            return console.error(err.toString());
        });
        ...
        data.append('toUsername', reciver);
        ...
        if (images.length > 0 || files.length > 0) {
            ...
            $.ajax({
                url: `/MessageChat/With/${reciver}/SendMessage`,
                processData: false,
                contentType: false,
                type: "POST",
                data: data,
